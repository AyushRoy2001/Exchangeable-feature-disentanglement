...
class conv_block(Layer):
    def __init__(self, filters):
        ...
        self.conv1 = Conv2D(self.filters, kernel_size=(3, 3), activation='relu', padding='same', use_bias=True, kernel_initializer='glorot_normal', bias_initializer=Constant(0.1))
        self.bn1 = BatchNormalization(epsilon=1e-3, beta_initializer=Constant(0.0), gamma_initializer=Constant(1.0), momentum=0.5)
        self.conv2 = Conv2D(self.filters, kernel_size=(3, 3), activation='relu', padding='same', use_bias=True, kernel_initializer='glorot_normal', bias_initializer=Constant(0.1))
        self.bn2 = BatchNormalization(epsilon=1e-3, beta_initializer=Constant(0.0), gamma_initializer=Constant(1.0), momentum=0.5)
...
class bottleneck_block(Layer):
    def __init__(self, filters):
        ...
        self.conv2 = Conv2D(self.filters, kernel_size=(3, 3), activation='relu', padding='same', use_bias=True, kernel_initializer='glorot_normal', bias_initializer=Constant(0.1))
        self.bn2 = BatchNormalization(epsilon=1e-3, beta_initializer=Constant(0.0), gamma_initializer=Constant(1.0), momentum=0.5)
        self.conv3 = Conv2D(self.filters, kernel_size=(3, 3), activation='relu', padding='same', use_bias=True, kernel_initializer='glorot_normal', bias_initializer=Constant(0.1))
        self.bn3 = BatchNormalization(epsilon=1e-3, beta_initializer=Constant(0.0), gamma_initializer=Constant(1.0), momentum=0.5)
...
class robust_residual_block(Layer):
    def __init__(self, filters):
        super(robust_residual_block, self).__init__()
        self.filters = filters
        self.conv1 = Conv2D(filters=self.filters, kernel_size=(3, 3), activation='relu', padding='same', use_bias=True, kernel_initializer='glorot_normal', bias_initializer=Constant(0.1))
        self.bn1 = BatchNormalization(epsilon=1e-3, beta_initializer=Constant(0.01), gamma_initializer=Constant(1.0), momentum=0.5)
        self.conv2 = SeparableConv2D(filters=self.filters, kernel_size=(3, 3), activation='relu', padding='same', use_bias=True, depthwise_initializer='glorot_uniform', pointwise_initializer='glorot_uniform', bias_initializer=Constant(0.1), depth_multiplier=1)
        self.bn2 = BatchNormalization(epsilon=1e-3, beta_initializer=Constant(0.01), gamma_initializer=Constant(1.0), momentum=0.5)
        self.conv3 = Conv2D(filters=self.filters, kernel_size=(3, 3), activation='relu', padding='same', use_bias=True, kernel_initializer='glorot_normal', bias_initializer=Constant(0.1))
        self.bn3 = BatchNormalization(epsilon=1e-3, beta_initializer=Constant(0.01), gamma_initializer=Constant(1.0), momentum=0.5)
        self.concat = Concatenate()
        self.conv4 = Conv2D(filters=self.filters, kernel_size=(3, 3), activation='relu', padding='same', use_bias=True, kernel_initializer='glorot_normal', bias_initializer=Constant(0.1))
        self.bn4 = BatchNormalization(epsilon=1e-3, beta_initializer=Constant(0.01), gamma_initializer=Constant(1.0), momentum=0.5)
...
def nuclei_segnet(input_shape, num_classes=1, output_activation='sigmoid'):
    ...
    x_conv1 = robust_residual_block(filters[0])(inputs)
    x_pool1 = MaxPooling2D((2, 2), strides=(2, 2), padding="same")(x_conv1)
    x_conv2 = robust_residual_block(filters[1])(x_pool1)
    x_pool2 = MaxPooling2D((2, 2), strides=(2, 2), padding="same")(x_conv2)
    x_conv3 = robust_residual_block(filters[2])(x_pool2)
    x_pool3 = MaxPooling2D((2, 2), strides=(2, 2), padding="same")(x_conv3)
    x_conv4 = robust_residual_block(filters[3])(x_pool3)
    x_pool4 = MaxPooling2D((2, 2), strides=(2, 2), padding="same")(x_conv4)
    x_conv5 = bottleneck_block(filters[4])(x_pool4)

    x_tconv5 = attention_decoder_block(x_conv5, filters[3], x_conv4)
    u_conv4 = conv_block(filters[3])(x_tconv5)
    x_tconv4 = attention_decoder_block(u_conv4, filters[2], x_conv3)
    u_conv3 = conv_block(filters[2])(x_tconv4)
    x_tconv3 = attention_decoder_block(u_conv3, filters[1], x_conv2)
    u_conv2 = conv_block(filters[1])(x_tconv3)
    x_tconv2 = attention_decoder_block(u_conv2, filters[0], x_conv1)
    u_conv1 = conv_block(filters[0])(x_tconv2)

    outputs = Conv2D(num_classes, (1, 1), padding='same', activation=output_activation, name='OUT')(u_conv1)

    model = Model(inputs=[inputs], outputs=[outputs,x_conv1,u_conv1,x_conv2,u_conv2,x_conv3,u_conv3,x_conv4,u_conv4,x_conv5])
...
modeller = Model(inputs=model.input, outputs=[model.get_layer(name="bottleneck_block").output,model.get_layer(name="conv_block").output,model.get_layer(name="conv_block_1").output,model.get_layer(name="conv_block_3").output])
